...
// 	}
    ...
    if("input".equals(method)){
		...
		if(cookies!=null&&cookies.length>0){
			for(int i =0; i<cookies.length; i++){
				if("codegen.author".equals(cookies[i].getName())){
    ...
    if("build".equals(method)||"packModule".equals(method)||"packProject".equals(method)){
		...
		omMap = builder.build();
		//为创建人设置cookies
		Cookie cookieAuthor = new Cookie("codegen.author", URLEncoder.encode(author,"UTF-8"));
		...
		if("packModule".equals(method)||"packProject".equals(method)){
	        ...
	        zu.setComment("该压缩包由Codegen代码生成器生成");
